...
- `defineConfig(schema, options?)` — synchronous load → validate → freeze → return.
- `defineDynamicConfig(schema, options)` — async loader for runtime-mutable values (feature flags, kill switches), with a `Proxy`-backed live view that reflects provider updates atomically.
- `t` — schema builder with primitives `string`, `number` (+ `int`), `boolean`, `port`, `url`, `email`, `enum`, `secret`, `json<T>()`.
- `Secret<T>` — leak-resistant wrapper covering every default leak surface.
- Source stack — built-in `envSource`, `dotenvSource` (auto-disabled in production), `cliSource` (`--app.port=8080` / `--app.port 8080`).
- Provider stack — `staticProvider` (single-snapshot, useful in tests), `pollingProvider` (BYO `fetch` + `intervalMs`, wraps LaunchDarkly / AppConfig / DB feeds).
- Diagnostics — aggregating box-drawn table renderer + `writeFailFast` (stderr + `exit(1)`).
- Observability — `defineConfig({ logger })` and `defineDynamicConfig({ logger })` emit structured boot-summary + dynamic-update lines via a **structurally-typed** `Logger`. No hard dependency on `forge/telemetry/log`.
- Testing — `mockConfig` (ALS-scoped overrides), `recordingProvider`, conformance scenarios for BYO provider implementations.
- Error taxonomy — `ConfigError` base + `ConfigValidationError` (with structured `issues[]`), `ConfigSourceError`, `ConfigSchemaError`, `ConfigSecretAccessError`, `ConfigProviderError`, `ConfigFrozenError`.
...
export const config = defineConfig({
  ...
  features: {
    rollout: t
      .json<{ readonly newCheckout: boolean }>()
      .default({ newCheckout: false }),
  },
});
...
The `Logger` parameter is **structurally typed** — any object with `info` / `warn` / `error` methods that accept `(msg, attrs)` works, including `console`. `forge/config` deliberately does _not_ import from `forge/telemetry/log`, so the module stays free of a hard telemetry dependency.
...
Built-in file support is `.env` only, and the config APIs are read-only. General JSON/YAML/TOML file sources and write/update persistence are out of scope.
